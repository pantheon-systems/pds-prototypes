...
function validateMetadata(metadata, projectDir) {
	...
	if (metadata.template && !VALID_TEMPLATES.includes(metadata.template)) {
		errors.push(
			`Invalid template: ${metadata.template}. Must be one of: ${VALID_TEMPLATES.join(', ')}`,
		);
	}
...
function aggregateMetadata() {
	...
	console.log(
		`Found ${directories.length} project director${directories.length === 1 ? 'y' : 'ies'}:\n`,
	);
	...
	directories.forEach((dir) => {
		...
		try {
			...
			console.error(
				`⚠️  ${dir.name}: Error reading metadata.json - ${error.message}`,
			);
			errorCount++;
		}
	});
	...
	if (errorCount > 0) {
		console.log(
			`⚠️  ${errorCount} project${errorCount === 1 ? '' : 's'} had errors\n`,
		);
	} else {
		console.log('');
	}
